...
class TestGcsJoin:
    @pytest.mark.parametrize(
        "include_protocol,expected_path",
        [(False, "path/to/bogus/file.txt"), (True, "gs://path/to/bogus/file.txt")],
    )
...
class TestGetClient:
    def test_works_with_service_account_key(self):
        ...
        mock_storage.Client.assert_called_once_with(
            project=GCP_PROJECT_NAME, credentials=fake_credentials
        )
...
def test_gcs_bucket_upload_download():
    with tempfile.TemporaryDirectory() as temp_path:
        ...
        upload_files(GCP_PROJECT_NAME, GCS_BUCKET_NAME, GCS_BUCKET_PATH, input_dir)
        ...
        try:
            ...
            for blob in blobs:
                assert blob.name in [
                    gcs_join([GCS_BUCKET_PATH, f])
                    for f in [filename0, filename1, filename2]
                ]
            ...
            # Find it in its new location
            blobs = list_bucket_contents(
                GCP_PROJECT_NAME, GCS_BUCKET_NAME, gcs_join([GCS_BUCKET_PATH, "copied"])
            )
            contents = []
            for blob in blobs:
                contents.append(blob.name)
            ...
            # ...all at once
            download_files(
                GCP_PROJECT_NAME, GCS_BUCKET_NAME, GCS_BUCKET_PATH, output_dir
            )
